...
@image_bp.route('/emojis', methods=['GET'])
def get_emojis():
    """获取所有表情包列表"""
    try:
...
@image_bp.route('/emoji/<emoji_id>', methods=['GET'])
...
@image_bp.route('/emoji/match', methods=['POST'])
def match_emojis():
    """根据用户消息匹配表情包"""
    try:
        data = request.json
...
@image_bp.route('/emoji/send', methods=['POST'])
